...
class DatabaseAdapter:
    ...
    def add_application(self, application: Application) -> int:
        ...
        if not application.metadata.created_at:
            application.metadata.created_at = datetime.now().strftime(
                "%Y-%m-%d %H:%M:%S"
            )
    ...
    def get_application(self, app_id: int) -> Optional[Application]:
        ...
        try:
            ...
            questions = [
                (row[0], row[1], row[2]) for row in questions_cursor.fetchall()
            ]
    ...
    def update_question(self, question_id: int, question: str, answer: str) -> bool:
        ...
        try:
            logger.info(
                "[DB] Updating question ID %d: Q='%s', A='%s'",
                question_id,
                question[:50],
                answer[:50],
            )
    ...
    def _row_to_application(
        self, row: sqlite3.Row, questions: List[tuple]
    ) -> Application:
        ...
        return Application(
            metadata=metadata, status=ApplicationStatus(row["status"]), id=row["id"]
        )
